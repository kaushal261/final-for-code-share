 ...
 * @param {boolean} [options.maskLines=true] - If true, each line is blurred
 *   until the student hovers (desktop) or taps (touch) that specific line.
 *   This means a screenshot only ever captures one revealed line at a time,
 *   raising the effort needed to reconstruct a whole file — still not a
 *   guarantee (see file header), but a meaningful extra deterrent.
 */
function renderProtectedCode(containerEl, code, language, options = {}) {
  const { maskLines = true } = options;
  containerEl.innerHTML = '';
  containerEl.classList.toggle('lines-masked', maskLines);
  ...
  if (maskLines) attachClickReveal(containerEl);
}

/**
 * On touch devices (no :hover), tapping a line toggles it revealed and
 * un-reveals any other line. Desktop hover is handled purely by CSS.
 */
function attachClickReveal(viewerEl) {
  if (viewerEl._revealBound) return;
  viewerEl._revealBound = true;
  viewerEl.addEventListener('click', (e) => {
    const line = e.target.closest('.code-line');
    if (!line) return;
    const wasRevealed = line.classList.contains('revealed');
    viewerEl.querySelectorAll('.code-line.revealed').forEach((l) => l.classList.remove('revealed'));
    if (!wasRevealed) line.classList.add('revealed');
  });
}
